    ...
    <section className="app-promo">
      <div className="promo-hero">
        <div className="contenitore app-promo-contenuto">
          <h1>{t.promoTitolo}</h1>
          <a className="app-store-button" href={APP_STORE_URL} rel="noreferrer" target="_blank" aria-label={t.scaricaAppStore}>
            ...
            <span><small>{t.scaricaSu}</small><strong>App Store</strong></span>
          </a>
        </div>
        <a className="scroll-cue" href="#screenshots" aria-label={t.scorriSchermate}>
          <svg aria-hidden="true" viewBox="0 0 24 24"><path d="m6 9 6 6 6-6" /></svg>
        </a>
      </div>

      <div id="screenshots" className="promo-gallery" aria-label={t.galleriaSchermate}>
        {SCREENSHOTS.map((screenshot) => (
          <img key={screenshot.src} src={screenshot.src} alt={t.schermataApp(screenshot.numero)} loading="lazy" />
        ))}
      </div>
    </section>
  )
